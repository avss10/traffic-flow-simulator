...
import com.traffic.flow.simulation.model._
import com.traffic.flow.simulation.openstreetmap.{Define, ReportHandler}
...
object Microscopic {
  ...
  def sim(sparkSession: SparkSession, edges: Dataset[Link], signals: Dataset[TrafficLight], intersects: Dataset[Intersect], vehicles: Dataset[MOBILVehicle],
    ...
    // TrafficFlowSim will do sample simulation to find the best repartition period with minimum time cost
